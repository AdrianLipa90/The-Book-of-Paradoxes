#!/usr/bin/env python3
"""Deterministic validator for the Dynamic Identity Invariant operator."""
...
def edge_transport(i, j):
    return mscale(1/sqrt(2*(1+dot(V[i], V[j]))), madd(I2, mm(S[i], S[j])))


def block_set(A, bi, bj, B):
    for i in range(2):
        for j in range(2):
            A[2*bi+i][2*bj+j] = B[i][j]
...
for a, b, t in [(0, 1, 0.31), (1, 2, 0.23), (2, 0, 0.19)]:
    ...
    block_set(H, b, a, mscale(-t, dagger(Uab)))
